...
function DataChannel(socket, property, on_data) {
  // this is all specific to phoenix sockets
  let channel = socket.channel(`data:${property}`);
  channel.join()
...
function Component(template, name, socket) {
  ...
  return React.createClass({
    ...
    componentWillMount: function() {
      DataChannel(socket, name, payload => {
...
let grouped = Array.prototype.reduce.call(components, (groups, comp) => {
  let kind = comp.attributes['data-component'].value;
  groups[kind] = groups[kind] || [];
  groups[kind].push(comp);
  return groups;
}, {});

let data = window.reactchetData;
...
for (let kind in grouped) {
  ...
  let component = Component(template, kind, socket);
  // There would end up being one of these for each "component" (or scope) in the view
...
module.exports.Ratchet = Ratchet;
