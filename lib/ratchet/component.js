...
function createComponent(node, data) {
  var attrs = attrsToProps(node.attributes);
  if (data.id) { attrs.key = data.id }
  var children = createChildren(node, data)
  return React.createElement.apply(undefined, [node.tagName, attrs].concat(children));
}
  ...
  return React.createClass({
    ...
    render: function() {
      ...
      if (Array.isArray(data)) {
        return React.createElement('div', null, data.map((d) => createComponent(template, d)));
      } else {
